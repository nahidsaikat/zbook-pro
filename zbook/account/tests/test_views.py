...
class TestAccountSubTypeListCreateAPIView:
    ...
    def test_create_created_by(self, auth_client, user, user2):
...
class TestAccountSubTypeRetrieveUpdateAPIView:
    ...
    def test_update_updated_by(self, auth_client, user, user2):
        subtype = AccountSubTypeFactory()
        data = factory.build(dict, FACTORY_CLASS=AccountSubTypeFactory, deleted=1, created_by=user2.pk)
...
class TestAccountRetrieveUpdateAPIView:
    ...
    def test_update_inactive(self, auth_client, user, sub_type):
        account = AccountFactory()
        data = factory.build(dict, FACTORY_CLASS=AccountFactory, created_by=user.pk, sub_type=sub_type.pk, inactive=1)
    ...
    def test_update_updated_by(self, auth_client, user, user2, sub_type):
        account = AccountFactory()
        data = factory.build(dict, FACTORY_CLASS=AccountFactory, created_by=user2.pk, sub_type=sub_type.pk, deleted=1)
