...
class TestVoucherRetrieveUpdateAPIView:
    ...
    def test_update_inactive(self, auth_client, user, sub_type, debit_account, credit_account):
        voucher = VoucherFactory(accounts=[debit_account, credit_account], created_by=user)
        voucher_number = fake.name()
        data = factory.build(dict, FACTORY_CLASS=VoucherFactory, voucher_number=voucher_number, sub_type=sub_type.pk,
                             type=sub_type.type, created_by=user.pk, inactive=True)
    ...
    def test_update_deleted(self, auth_client, user, sub_type, debit_account, credit_account):
        voucher = VoucherFactory(accounts=[debit_account, credit_account], created_by=user)
        voucher_number = fake.name()
        data = factory.build(dict, FACTORY_CLASS=VoucherFactory, voucher_number=voucher_number, sub_type=sub_type.pk,
    ...
    def test_update_unauthorize(self, client, user, sub_type, debit_account, credit_account):
        voucher = VoucherFactory(accounts=[debit_account, credit_account], created_by=user)
        voucher_number = fake.name()
        data = factory.build(dict, FACTORY_CLASS=VoucherFactory, voucher_number=voucher_number, sub_type=sub_type.pk,
                             type=sub_type.type, created_by=user.pk, deleted=True)
    ...
    def test_get_voucher_unauthorize(self, client, user, debit_account, credit_account):
        voucher = VoucherFactory(accounts=[debit_account, credit_account], created_by=user)
        url = reverse('voucher:detail-update', args=[voucher.pk])
